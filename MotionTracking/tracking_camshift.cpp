/**
 * @file tracking_camshift.cpp
 * @author Shinichi Inoue (inoue.shinichi.1800@gmail.com)
 * @brief CAMShift(Continuously Adaptive Meanshift)による物体トラッキング
 * @version 0.1
 * @date 2023-08-13
 *
...
int main(int argc, char **argv)
{
    try
    {
        ...
        // CAMShift
        cv::Point2f track_pts[4]; // 4 points
        cv::RotatedRect tracking_roi;
        cv::Rect initial_window;
        initial_window.x = 0;
        initial_window.y = 0;
        initial_window.width = 250;
        initial_window.height = 250;
                                      ...
                                      const cv::Mat &lastFrame) -> void
        {
            ...
            /* CAMShift */
            tracking_roi = cv::CamShift(img_bin,
                                        /*cv::Rect*/ initial_window,
                                        /*cv::TermCriteria*/ criteria);

            tracking_roi.points(track_pts);
            ...
            for (int i = 0; i < 4; i++)
            {
                cv::line(img_dst, track_pts[i], track_pts[(i + 1) % 4], cv::Scalar(255, 0, 0), /*thickness*/3);
            }
