...
namespace is
{
    namespace common
    {
         ...
         * @return std::wstring
         */
        std::wstring cvt_shiftjis_to_utf16(const char *shiftjis_char)
        {
            if (!shiftjis_char)
                return std::wstring();

            size_t necessary_size = ::MultiByteToWideChar(CP_ACP, 0, shiftjis_char, -1, NULL, 0);
            std::wstring utf16_wstr(necessary_size, 0);
            ::MultiByteToWideChar(CP_ACP, 0, shiftjis_char, -1, &utf16_wstr[0], necessary_size);
            utf16_wstr.resize(necessary_size - 1); // '\0'を消す
            return utf16_wstr;
        }
         ...
         * @brief Shift-JIS -> UTF-16
         *
         * @param shiftjis_string
         * @return std::wstring
         */
        std::wstring cvt_shiftjis_to_utf16(const std::string &shiftjis_string)
        {
            return cvt_shiftjis_to_utf16(shiftjis_string.c_str());
        }
        ...
        std::string cvt_utf16_to_shiftjis(const wchar_t *utf16_wchar)
        {
            ...
            std::string shiftjis_str(necessary_size, 0);
            ::WideCharToMultiByte(CP_ACP, 0, utf16_wchar, -1, &shiftjis_str[0], necessary_size, NULL, NULL);
            shiftjis_str.resize(necessary_size - 1); // '\0'を消す
            return shiftjis_str;
        }
         ...
         * @return std::string
         */
        std::string cvt_shiftjis_to_utf8(const char *shiftjis_char)
        {
            /* Shift-JIS -> UTF-16 -> UTF-8 */
            std::wstring utf16_wstr = cvt_shiftjis_to_utf16(shiftjis_char);
            return cvt_utf16_to_shiftjis(utf16_wstr);
        }
         ...
         * @return std::string
         */
        std::string cvt_shiftjis_to_utf8(const std::string &shiftjis_string)
        {
            /* Shift-JIS -> UTF-16 -> UTF-8 */
            std::wstring utf16_wstr = cvt_shiftjis_to_utf16(shiftjis_string);
            return cvt_utf16_to_shiftjis(utf16_wstr);
        }
    }
}
